...
class CornersProblem(search.SearchProblem):
    ...
    def successorStates(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                cornersLeft = list(state[1])  # fetching the parent's corners list
                if state[0] in self.corners and state[0] in cornersLeft:
                    cornersLeft.remove(state[0])
                    succ.append(((nextState, tuple(cornersLeft)), action, 1))
                else:
                    succ.append(((nextState, state[1]), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        ...
        dx = abs(x0 - x1)
        dy = abs(y0 - y1)
        cornerDist[corner] = (dx + dy)
    return min(cornerDist.itervalues())
...
def foodHeuristic(state, problem):
    ...
    "*** Your Code Here ***"
    if problem.heuristicInfo.has_key('food_list'):
